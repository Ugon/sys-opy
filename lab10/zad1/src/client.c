...
pthread_t prolongingThread, ioThread;

void* prolongFunc(void *arg) {
	struct msg prolongMsg;
	prolongMsg.type = MSG_TYPE_PROLONG;
	strcpy(prolongMsg.name, name);
	while(8) {
		sleep(PROLONG_FREQUENCY);
		if (write(pollFD.fd, &prolongMsg, sizeof(struct msg)) == -1)                          { printf("write() fail, %s", strerror(errno)); return NULL; }
	}
}

void* ioFunc(void *arg) {
	struct msg toSendMsg;
	toSendMsg.type = MSG_TYPE_MSG;
...
int main(int argc, char** argv) {
	...
	if ((pollFD.fd = socket(addr->sa_family, SOCK_DGRAM, 0)) == -1)                            { printf("socket() fail, %s", strerror(errno)); return -1; }
	...
	struct msg registerMsg;
	registerMsg.type = MSG_TYPE_REGISTER;
	strcpy(registerMsg.name, name);

	if (write(pollFD.fd, &registerMsg, sizeof(struct msg)) == -1)                              { printf("write() fail, %s", strerror(errno)); return -1; }

	if (pthread_create(&prolongingThread, NULL, prolongFunc, NULL) != 0 )                      { printf("pthread_create() fail, %s", strerror(errno)); return -1; }
	if (pthread_create(&ioThread, NULL, ioFunc, NULL) != 0 )                                   { printf("pthread_create() fail, %s", strerror(errno)); return -1; }

	while (loop) {
		int events = 0;
		if ((events = poll(&pollFD, 1, -1)) == 0)                                              { printf("pool() fail, no events\n"); continue; }
		else if (events == -1) {
			...
			if (receivedMsg.type == MSG_TYPE_MSG) {
				printf("%s: %s\n", receivedMsg.name, receivedMsg.txt);
				continue;
			}
			
			if (receivedMsg.type == MSG_TYPE_REGISTER) {
